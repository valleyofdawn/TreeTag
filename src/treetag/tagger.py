...
def TreeTag(
    ...
    def _must_exist(p: str | Path, what: str) -> str:
        if not isinstance(p, (str, Path)) or not Path(p).exists():
            raise FileNotFoundError(
                f"{what} not found: {p!r}. Pass a real path; defaults (None) are not supported."
            )
        return str(p)
    ...
    def _ensembl_fraction(names, sample=200):
        ...
        idx = np.random.default_rng(0).choice(
            len(names), size=min(sample, len(names)), replace=False
        )
        s = [str(names[i]) for i in idx]
        return sum(
            x.startswith(("ENSG", "ENSMUSG", "ENSDARG", "ENS")) for x in s
        ) / max(1, len(s))
    ...
    def _build_weights(used_cols, pos_lists, neg_lists, k=None):
        ...
        W_neg = (
            coo_matrix((data_n, (rows_n, cols_n)), shape=(n_local, k)).tocsr()
            if data_n
            else None
        )
        return W_pos, W_neg
    ...
    def _prune_debug(
        c, child_list, counts, means_used, union_pos, drop_cols, marker_union
    ):
        """Verbose diagnostics for pruned markers at a split."""
        try:
            ct_str = ", ".join(
                f"{child_list[ii]}:{int(counts[ii])}" for ii in range(len(child_list))
            )
            ...
            for col_global in drop_cols:
                ...
                EPS_dbg = 1e-9
                fc_dbg = (
                    (float(child_val) + EPS_dbg) / (float(other_mean) + EPS_dbg)
                    if np.isfinite(child_val)
                    else float("nan")
                )
                vals_fmt = "; ".join(
                    f"{child_list[i]}={float(v):.4f}"
                    if np.isfinite(v)
                    else f"{child_list[i]}=nan"
                ...
                _log(
                    f"   - {gene}: {vals_fmt} | other_avg={other_str}, child={child_str}, FC(child/others)={fc_dbg:.3f}"
                )
    ...
    marker_union = sorted(
        {g for v in subtree_vertices for g in (v["pos_markers"] + v["neg_markers"])}
    )
    ...
    def _get_scaled_marker_csc():
        ...
        if not cols:
            ...
            if frac >= 0.5:
                _log(
                    "[TreeTag] No marker overlap; IDs look like Ensembl (>=50%). Trying auto-convert…"
                )
                info = convert(
                    adata, prefer_var_cols=("feature_name", "gene_symbols", "SYMBOL")
                )
                var_names = adata.raw.var_names if use_raw else adata.var_names
                col_map = {g: i for i, g in enumerate(var_names)}
                cols = [col_map[g] for g in marker_union if g in col_map]
                if not cols:
                    raise RuntimeError(
                        "After Ensembl→symbol conversion, still zero overlap. Check your marker YAML."
                    )
                _log(
                    f"[TreeTag] Auto-converted (used={info.get('used')}, changed={info.get('changed')})."
                )
            else:
        ...
        X_csc = (
            X_mark
            if (issparse(X_mark) and getattr(X_mark, "format", "") == "csc")
            else csc_matrix(X_mark)
        )
        ...
        _log(
            f"[TreeTag] prepared {msg} marker CSC in {time.perf_counter()-t0:.2f}s (cells={X_csc.shape[0]}, genes={X_csc.shape[1]})"
        )
        return X_csc
    ...
    if smoothing or majority_vote:
        ...
        def _neighbors_matrix():
            ...
            if conn_key and conn_key in adata.obsp:
                C_raw = adata.obsp[conn_key]
                src = conn_key
            elif "connectivities" in adata.obsp:
                C_raw = adata.obsp["connectivities"]
                src = "connectivities"
            else:
                ...
                src = adata.uns.get("neighbors", {}).get(
            ...
            nb_token = (
                ...
                src,
                C_raw.shape,
                int(getattr(C_raw, "nnz", C_raw.size)),
            )
            ...
            if C_raw.shape[0] != adata.n_obs:
                raise ValueError(
                    "neighbor connectivities shape mismatch with adata.n_obs"
                )
    ...
    def _prepare_node(node, kids):
        ...
        node_cache[node] = dict(
            ...
            child_score_names=child_score_names,
            W_pos=W_pos,
            W_neg=W_neg,
        )
        return node_cache[node]
    ...
    def _prune_pos_markers_min_fc(row_idx, child_list, child_assignments):
        ...
        if (not _do_prune) or len(child_list) <= 1 or row_idx.size == 0:
            kept = {
                c: [g2col[g] for g in G.vs[vmap[c]]["pos_markers"] if g in g2col]
                for c in child_list
            }
        ...
        counts = np.array(
            [np.count_nonzero(child_assignments == j) for j in range(k)], dtype=int
        )
        ...
        for c_idx, c in enumerate(child_list):
            ...
            if counts[c_idx] == 0 or not cols_c:
                ...
                why = (
                    "0 cells assigned"
                    if counts[c_idx] == 0
                    else "no candidate +markers present"
                )
                _log(
                    f"[Prune] {c}: skipped pruning ({why}). Kept {len(cols_c)} markers."
                )
                continue

            pos_idx_in_union = [union_pos[col] for col in cols_c]
            child_mean = means_used[c_idx, pos_idx_in_union]  # (m,)
            others_mean = (sum_mu[pos_idx_in_union] - child_mean) / max(
                1, k - 1
            )  # (m,)
            ...
            _log(
                f"[Prune] {c}: kept {len(kept_cols)}/{len(cols_c)} (+markers, FC vs avg(others) ≥ {min_pruning_fc:g})."
            )
            if pruned_log[c]:
            ...
            if len(kept_cols) < hard_min:
                msg = (
                    f"[Prune:STOP] {c} has {len(kept_cols)} positive markers after pruning "
    ...
    def _score_children_rows(row_idx, node, kids):
        ...
        if _do_prune and len(child_list) > 1:
            ...
            neg_lists = [
                [g2col[g] for g in G.vs[vmap[c]]["neg_markers"] if g in g2col]
                for c in child_list
            ]
            ...
            for lst in pos_lists_pruned + neg_lists:
                for gidx in lst:
                    if gidx not in seen:
                        seen.add(gidx)
                        used_cols2.append(gidx)

            if not used_cols2:
                return (
                    child_list,
                    child_score_names,
                    np.zeros((row_idx.size, 0), dtype=np.float32),
                    0,
                )

            W_pos2, W_neg2 = _build_weights(
                used_cols2, pos_lists_pruned, neg_lists, k=len(child_list)
            )
    ...
    def _walk(node, row_idx):
        ...
        if not kids or row_idx.size == 0:
            ...
            return
        t0 = time.perf_counter()
        child_names, child_score_names, S, n_local = _score_children_rows(
            row_idx, node, kids
        )
        t1 = time.perf_counter()
        _log(
            f"[TreeTag] split @ '{node}' (cells={row_idx.size}, X_used={row_idx.size}x{n_local}, score={t1 - t0:.2f}s, children_scored={len(child_names)})"
        )
        if S.shape[1] == 0:
            labels.iloc[row_idx] = node
            return
    ...
    if majority_vote:
        if C_full is None:
            raise ValueError(
                "majority_vote requested but neighbor connectivities were not prepared"
            )
        label_to_idx = {lab: i for i, lab in enumerate(cats)}
        idx = np.fromiter(
            (
                label_to_idx.get(lab, -1)
                for lab in adata.obs["TreeTag"].astype(str).values
            ),
            dtype=int,
        )
